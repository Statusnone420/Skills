...
A synthetic Cline-shaped regression records the original failures and their repair. Fresh group proof covers all 770 current tests: 199 core, 392 lifecycle, and 179 trajectory tests passed on both Windows and WSL Ubuntu. The latest Windows timings were 30 seconds for core, 465 seconds for lifecycle, and 25 seconds for trajectory; native ext4 Ubuntu took 3, 38, and 14 seconds respectively. The same observable runner, exact 34-module partition, Python 3.14 pin, and three test groups now drive GitHub Actions. Adapter regeneration/parity and the repository checker remain required closeout gates.
