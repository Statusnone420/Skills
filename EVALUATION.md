...
A synthetic Cline-shaped regression records the original failures and their repair. Fresh group proof covers all 767 current tests: 196 core, 392 lifecycle, and 179 trajectory tests passed on both Windows and WSL Ubuntu. The latest Windows timings were 37 seconds for core, 479 seconds for lifecycle, and 28 seconds for trajectory; native ext4 Ubuntu took 6, 51, and 16 seconds respectively. The same observable runner, exact 34-module partition, Python 3.14 pin, and three test groups now drive GitHub Actions. Adapter regeneration/parity and the repository checker remain required closeout gates.
