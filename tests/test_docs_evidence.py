...
class EvidenceReceiptTests(unittest.TestCase):
    ...
    def test_orientation_marks_comment_only_frontmatter_title_unavailable(self):
        scenarios = {
            "comment-only": (
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for name, (source, expected) in scenarios.items():
                ...
                observed = evidence.observe_entry_orientation(root, entry.name)
                with self.subTest(name=name):
                    self.assertEqual(observed["frontmatter_title"], expected)
...
class CorpusHarnessTests(unittest.TestCase):
    ...
    def test_runner_atomic_output_accepts_near_limit_basename(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            workspace = root / "workspace"
            workspace.mkdir()
            output = root / ("x" * 242)
            output.write_text("previous bytes\n", encoding="utf-8")
            result = {"schema_version": 1, "repositories": []}
            with mock.patch.object(
                run_docs_corpus, "run_corpus", return_value=result
            ):
