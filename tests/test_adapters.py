...
class AdapterBuilderTests(unittest.TestCase):
    ...
    def test_check_detects_version_drift_across_generated_surfaces(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            wrapper_path.write_text(wrapper_path.read_text(encoding="utf-8").replace("v0.1.2", "v9.9.9"), encoding="utf-8")
    ...
    def test_check_detects_static_skill_version_drift(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            skill_path.write_text(
                skill_path.read_text(encoding="utf-8").replace('version: "0.1.2"', 'version: "9.9.9"'),
                encoding="utf-8",
            )
