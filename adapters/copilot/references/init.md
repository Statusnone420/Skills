...
Route the bounded canonical request JSON through the installed closeout entrypoint on stdin.
For the zero-write preview, set `operation` to `preview` and use the exact command for the
active shell. On POSIX:
