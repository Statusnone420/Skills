...
`init`: perform a one-time repository adoption whose size follows the diagnosed repository condition. Its initial response is a complete zero-write adoption preview. Follow the single detailed [Init interaction contract](init.md); the LLM running the skill owns bounded continuation and asks only at genuine ambiguity or the exact approval boundary.
