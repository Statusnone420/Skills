...
If already on a verified clean feature branch, use it. Do not create a worktree or say that one exists. Only report a worktree that was actually created. Otherwise, prefer a safe worktree; use a feature branch only after verifying it excludes unrelated dirty changes.
